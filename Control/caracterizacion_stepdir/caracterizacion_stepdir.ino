...
int angulo_servo = 90; // ángulo inicial
...
void setup() {
  ...
  tiempo_prev = millis();
  tiempo_inicio_loop = millis(); // Guardamos el momento exacto en que inicia la prueba
}
...
void loop() {
  ...
  if (!escalon_activado && (millis() - tiempo_inicio_loop > 3000)) {   // el servo se mueve tras 3 segundos
    angulo_servo = 0;
    miServo.write(angulo_servo);
    escalon_activado = true; // se cierra el loop del escalon
  }

  if (millis() - tiempo_impresion > 50) { // tiempo entre la imprensión de cada dato nuevo
